...
# ==========================================================
# 触摸屏读取封装
# 不同 K230 固件触摸 API 名字可能不同。
# 如果你的固件触摸类不是 TOUCH，需要只改这里。
# ==========================================================
class TouchReader:
    def __init__(self, display_size):
        ...
        self.fail_limit = 5
        self.last_read_ms = 0
        self.read_interval_ms = 30

        try:
            from machine import TOUCH

            self.tp = TOUCH(0)
            self.event_down = getattr(TOUCH, "EVENT_DOWN", None)
            self.event_move = getattr(TOUCH, "EVENT_MOVE", None)
            self.event_up = getattr(TOUCH, "EVENT_UP", None)
            self.ok = True
            print("Touch init ok: machine.TOUCH")
    ...
    def parse_point(self, p):
        ...
        try:
            ...
            if event == self.event_up:
                return False, -1, -1

            if event == self.event_down or event == self.event_move:
                return True, x, y
    ...
    def read_raw(self):
        ...
        如果没有触摸，返回 False, -1, -1
        """
        if self.disabled or not self.ok or self.tp is None:
            return False, -1, -1
        ...
        try:
            ...
            # K230 示例形式: tp.read(1) 返回触摸点列表。
            if isinstance(points, tuple) or isinstance(points, list):
                if len(points) == 0:
                    return False, -1, -1

                state = self.parse_point(points[0])
            ...
            if self.fail_count >= self.fail_limit:
                self.disabled = True
                self.last_pressed = False
                print("Touch disabled after repeated read failures")

            return False, -1, -1

    def get_click(self):
        """
        只在按下瞬间返回一次点击，避免长按重复触发。
        """
        if self.disabled:
            return False, -1, -1
        ...
        try:
            now = time.ticks_ms()
            if time.ticks_diff(now, self.last_read_ms) < self.read_interval_ms:
                return False, -1, -1
            self.last_read_ms = now
        except Exception:
            pass

        pressed, x, y = self.read_raw()
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            print("Initial home draw failed:", e)
        ...
        while True:
            with ScopedTiming("total", 1):
                ...
                if ui.mode == ui.MODE_HOME:
                    ...
                    # 先显示首页，再读取触摸，避免触摸异常导致首屏黑屏。
                    if touch is not None:
                        clicked, tx, ty = touch.get_click()
                    else:
                        clicked, tx, ty = False, -1, -1
